...
public class Dao {
	...
	// Affiche des commentaires en dur actuellement, doit être MAJ suivant les classes des autres.
		...
		public static ArrayList<Abonne> initAbo() {
			...
			Abonne abonne1 = new Abonne("Cole", "James", "cole", "pasdemail@gmail.com", "0607080910", "password", "NMD-123" );
			Abonne abonne2 = new Abonne("Pasdenom", "pasdePrenom", "user", "user@gmail.com", "0607030210", "password", "DAD-456" );
		...
		public static String aboSelect() {
			String select = "<select name=\"nom\" id=\"abolist\">";
			select+= "<option value=\"\">--Choisissez un abonné--</option>";
			for (Abonne abonne : abonnes) {
				select+= "<option value=\""+abonne.getNom()+"\">, "+abonne.getPrenom()+"</option>";
			}
			return select;
		}
		
		public static String adminSelect() {
			String select = "<select name=\"nom\" id=\"abolist\">";
			select+= "<option value=\"\">--Choisissez un abonné--</option>";
			for (Admin admin : admins) {
				select+= "<option value=\""+admin.getNom()+"\">, "+admin.getPrenom()+"</option>";
			}
			return select;
		}
}
