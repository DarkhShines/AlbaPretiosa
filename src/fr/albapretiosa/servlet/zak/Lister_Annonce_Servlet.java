...
public class Lister_Annonce_Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Annonces annonce_liste = new Annonces();
		
		if(a != null) {
			for(Annonce an : Dao.getAllAnnonceById(a)) {
				if(an.getAbonne().getIdAbonne() == a.getIdAbonne() ) {
					annonce_liste.add(an);
				}
			}
			request.setAttribute("liste", annonce_liste);
			System.out.println(annonce_liste);
			
			String context = request.getContextPath();
			String path    = "/vue/ListerAnnonce.jsp";
